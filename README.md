...
- 没检测到的工具/目录会自动置灰，不能勾。
- 标「管理员」的项需要管理员权限：执行时调用 `pkexec`，由系统弹出 polkit 密码框（需要桌面环境的 polkit agent，绝大多数 Linux 桌面默认都有；命令行环境请用 CLI）。
- 标「敏感」的项删的是用户数据 / 配置 / 系统状态。GUI 里「勾选」即代表你已确认要删它，点按钮后还会再弹一次汇总确认。
